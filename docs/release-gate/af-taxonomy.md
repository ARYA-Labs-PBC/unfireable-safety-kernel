...
Status: canonical taxonomy for the [internal-ref]((internal issue)) release-gate adversarial suite. Authored as the planner/architect output for the AF-seed wave (2026-05-30).

## Why this document exists

The [internal-ref]((internal issue)) description names a 7-class AF taxonomy: `AF-contracts`, `AF-sdk`, `AF-image`, `AF-reconciler`, `AF-tlog`, `AF-tee`, `AF-key`. The repo already ships substantial adversarial fixture scaffolding using a different organizing axis — the **internal-ref SDK 6-fixture set** plus **per-component purple-team campaign letters** (A1, B1, C1, C2, D1, D2, F1, F2, G1a/b, etc.).
...
| **AF-tee** | TEE attestation forgery — quote signed by attacker, replayed quote, runtime measurement mismatch. **Deferred to v2.0** per [internal-ref]((internal issue)); no TEE in v1.0 commodity-hardware target. Stub fixture documents the deferral. | n/a in v1.0 |
| **AF-key** | Operator-key custody / rotation / revocation violation — key issued before the rotation start time accepted past the overlap window, revoked key accepted, dual-sign overlap window honored. | `crates/services/safety-kernel/src/key_*.rs` + internal-ref Step 14R |
...
| AF-sdk | ✅ `safety_kernel_client/tests/adversarial.rs` (5 internal-ref fixtures), `safety_kernel_client/tests/purple/adversarial_campaigns.rs` | ✅ `examples/testing/adversarial_fixtures.py` (6 fixtures) | ✅ STRONG |
...
|---|:---:|---|
| AF-tee | **Yes** | TEE / TDX / SEV-SNP attestation is out of scope for the v1.0 commodity-hardware target (internal-ref). The deployment surface AF-tee attacks does not exist in v1.0. |
...
- **Adversarial fixture *content*** beyond seeds. The seed wave proves the taxonomy + coverage script. internal-ref / 1886 / 1889 / 1890 populate to **full coverage** per internal-ref's release-gate timeline. The seed wave is "skeleton with proof-of-rejection per class," not "exhaustive attack matrix."
- **Cross-language byte-identical fixture parity.** The existing Python `adversarial_fixtures.py` claims byte-identical IDs across Rust+Python; this property is preserved for the AC16 6-fixture set but does NOT extend to every AF class. Cross-language **structural** parity (every class has both) is the seed-wave requirement; **byte-identical fixture IDs** is a v1.0-release-gate requirement that internal-ref/86/89/90 will deliver.
...
- Existing adversarial test suites still green (the seed wave does not break the AC16/internal-ref/transparency-log campaigns).
...
## Post-/purple-team hardening (internal-ref, internal-ref)
