# /test QA report — internal-ref AC1 (AF taxonomy seed)

**Session ID:** `d46f1b8ac99e885a74badc48`
**Wave:** internal-ref AC1 — AF taxonomy + seed fixtures + coverage script
...
| AF-tee | `tests/adversarial/seed/af_tee_DEFERRED.md` | ⏸ deferred to v2.0 (no TEE in v1.0 hardware target per internal-ref) | n/a | DEFERRED |
...
1. **AF-image production defence is structural-lint-only.** A real supply-chain attack might pass the lint (correct shape, but backdoored binary in the COPY). Full cryptographic build-provenance verification (sigstore + SLSA attestation chain) is internal-ref / internal-ref follow-up.
...
```jsonl
{"session_id": "d46f1b8ac99e885a74badc48", "wave": "internal-ref-AC1", "skill": "test", "verdict": "PASS", "fixtures": {"python": 12, "rust": 5}, "coverage_script": {"positive_exit": 0, "negative_exit": 1}, "rule_8": "pass", "rule_9": "pass", "timestamp_utc": "2026-05-30"}
```
