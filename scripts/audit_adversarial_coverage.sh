#!/usr/bin/env bash
# audit_adversarial_coverage.sh — enforce the internal-ref AF taxonomy.
#
...
# counted — this closes the internal-ref F2 gate-bypass (a no-op fixture
...
# internal-ref F3 fix: only these classes may be satisfied by a deferral
...
# internal-ref F2 fix: a file counts as a fixture only if it actually
...
# internal-ref F1/F4 fix: a fixture is "prod-exercised" if it imports a
...
for class in "${AF_CLASSES[@]}"; do
  ...
  if [ -f "${deferral}" ]; then
    if is_deferrable "${class}"; then
      ...
      # internal-ref F3: a deferral marker for a non-deferrable class is an
  ...
  if [ "${rust_count}" -eq 0 ] || [ "${py_count}" -eq 0 ]; then
    ...
    # internal-ref F1/F4: surface prod-exercised vs seed-model-only so a
