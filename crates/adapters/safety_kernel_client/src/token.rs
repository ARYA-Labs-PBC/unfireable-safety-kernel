...
//!
//! internal-ref Phase 4 / internal-ref AC9: the client SDK MUST reject any
...
impl PinnedKeyVerifier {
    ...
    /// installed kernel version (internal-ref AC8).
    ///
    /// # Errors
    ///
    ...
    ///
    /// **Audience claim (`expected_aud`)**: per PT-S2-M1 (internal-ref
    ...
    /// `expected_aud`. PT-S2-M1 (internal-ref slice 5) — closes the
...
mod tests {
    ...
    fn pinned_verifier_rejects_token_signed_with_attacker_key() {
        // internal-ref AC9 / internal-ref Migration Note AC8 (R): the verifier
    ...
    fn pinned_verifier_rejects_expired_token() {
        // internal-ref AC14: replay of an expired token must be rejected.
        let (signing, public) = fixed_keypair();
        let now = 1_700_000_000.0;
        let token = sign_kernel_token(&sample_claims(now), &signing);
