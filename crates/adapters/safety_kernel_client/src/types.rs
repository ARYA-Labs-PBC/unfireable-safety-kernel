...
//!
//! internal-ref Phase 2a Step 2 — kernel-decision pure types moved to
...
pub struct AuthorizeRequest {
    ...
    /// internal-ref). `#[serde(skip)]` keeps this field strictly out of
...
///
/// internal-ref Phase 2a Step 2 / Addendum 2a §4 — promoted from the
...
pub enum KernelClientError {
    ...
    /// substituted kernel (internal-ref Phase 4 / internal-ref key-pinning AC).
...
// internal-ref Phase 2a Step 2 — the pre-promotion `KernelError` enum has
...
mod tests {
    ...
    fn authorize_request_traceparent_omitted_when_none() {
        // Per internal-ref W4, traceparent is optional. JSON output must
    ...
    fn unavailable_error_is_fail_closed_path() {
        // internal-ref AC2 (R): Unavailable must NOT be confusable with
