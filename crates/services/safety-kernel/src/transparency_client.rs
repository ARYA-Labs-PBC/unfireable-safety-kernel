...
pub enum TransparencyError {
    ...
    /// compromised or buggy — kernel fails CLOSED. internal-ref / Phase 3
...
impl ReqwestTransparencyClient {
    ...
    ///
    /// internal-ref / Phase 3 Step 8: the kernel presents this identity
...
struct AppendResponseBody {
    ...
    /// t-log claims it stored. internal-ref added a kernel-side cross-check
...
impl TransparencyClient for ReqwestTransparencyClient {
    async fn append(
        ...
        if status.is_success() {
            ...
            // internal-ref / Phase 3 Step 8 — cross-verify that the
...
/// (internal-ref). Either-side missing keeps the legacy x-api-key-only
...
mod tests {
    ...
    // internal-ref — mTLS identity loader tests.
    ...
    fn client_builder_loads_pem_identity_pair_ok() {
        // internal-ref happy-path: a valid cert+key pair must build a
    ...
    fn client_builder_missing_cert_path_in_prod_errors() {
        // internal-ref fail-closed: pointing at a path that doesn't exist
    ...
    fn client_builder_malformed_pem_errors() {
        // internal-ref fail-closed: invalid PEM bytes must error at builder
